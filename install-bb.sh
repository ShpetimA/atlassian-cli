...
# Get latest release tag for bb
LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/${REPO}/releases" | grep -o '"tag_name": "bb-v[^"]*"' | head -1 | cut -d'"' -f4)
if [ -z "$LATEST_TAG" ]; then
  echo "Error: Could not find latest bb release"
  exit 1
fi
